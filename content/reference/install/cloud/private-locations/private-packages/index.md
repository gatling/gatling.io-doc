...
The control plane has a server to manage uploads to the repository, secured by a Gatling Enterprise API Token with `Configure` role.

The server is accessible on port 8080 by default, and provides a healthcheck endpoints via the `/info` path. 
The other routes are only enabled if the repository is set up.
...
- **server.port**: The port on which the control plane is listening.
...
- **repository.upload.directory**: This directory temporarily stores uploaded JAR files. (optional)
...
- Using an Application Load Balancer (ALB) (Recommended for production)
  ...
  - If you optionally wish to implement TLS encryption on the traffic between ALB and Control Plane server, generate a certificate for the server and update [server configuration]({{< ref "#control-plane-server" >}}) in the Control Plane configuration with the generated certificate.
...
- Direct IP Aliasing
  ...
  - Update the [server configuration]({{< ref "#control-plane-server" >}}) in the Control Plane configuration with the generated certificate.
...
- Using a Google Cloud HTTPS Load Balancer (Recommended for production)
  ...
  - If you optionally wish to implement TLS encryption on the traffic between Google Cloud HTTPS Load Balancer and Control Plane server, generate a certificate for the server and update [server configuration]({{< ref "#control-plane-server" >}}) in the Control Plane configuration with the generated certificate.
