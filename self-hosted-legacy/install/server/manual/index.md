...
seotitle: Manually install Gatling Enterprise Edition Self-Hosted
description: Manually install or upgrade Gatling Enterprise Edition, and learn how to configure it.
lead: Manually install or upgrade Gatling Enterprise Edition, and learn how to configure it.
...
{{< alert warning >}}
Your current Gatling Enterprise Edition version must be at least 1.6.2. Otherwise, you must first upgrade to Gatling Enterprise Edition 1.11.1.
{{< /alert >}}

Inside the Gatling Enterprise Edition installation folder, there are two directories to backup:
...
* Stop Gatling Enterprise Edition

It can be done by removing a file named `FRONTLINE_RUNNING` in the Gatling Enterprise Edition installation folder and waiting 10 seconds.
...
* [Download and unzip updated Gatling Enterprise Edition bundle]({{< ref "#download-gatling-enterprise" >}})
...
* Start the new Gatling Enterprise Edition process
...
If you are currently using Cassandra 3 with Gatling Enterprise Edition, consider upgrading to the latest version of Cassandra 4.
...
We support both AMD64 and ARM for both Gatling Enterprise Edition and Gatling injectors.

## JDK

Gatling Enterprise Edition components run on a JVM and require a modern Hotspot-based JDK 8, 11 or 17.
...
- JDK 11 and 17 support is limited to Gatling Enterprise Edition server and Gatling injectors only.
{{< /alert >}}

## Linux

{{< alert warning >}}
- the Gatling Enterprise Edition server and the Gatling injectors are intended to be running on Linux 64 bits.
...
As Gatling Enterprise Edition is about duration measurement and logging events in time, we advice that:
...
{{< alert warning >}}
We recommend you to perform regular backups of your data, all the more prior to upgrading Gatling Enterprise Edition to a new version.
{{< /alert >}}
...
Gatling Enterprise Edition has been tested against Cassandra 3.10 to 5.0.
If possible, we advise you go with the latest stable Cassandra 5.0 version.
...
For an initial evaluation, you can host Gatling Enterprise Edition and Cassandra on the same host.

If hosting Gatling Enterprise Edition and Cassandra on the same instance, consider a 8 cores (4 cores with hyper-threading) host with 16Gb of RAM and 20Go of disk space.
...
Gatling Enterprise Edition itself requires little disk space, as only uploaded private keys are stored on Gatling Enterprise Edition's host.
...
- C5d are equipped with NVMe SSD, which are great for installation running Cassandra and Gatling Enterprise Edition on the same virtual machine
- I3 are better for virtual machines reserved for Cassandra

In Azure, use instances with the `s` prefix:

- FsV2 are great for installations running Cassandra and Gatling Enterprise Edition on the same virtual machine
...
Finally, remember the host you configured, as you will need it later to configure the contact points of Gatling Enterprise Edition.
Keyspace creation will be handled by Gatling Enterprise Edition.
...
The Gatling Enterprise Edition host needs network access to:
...
## Gatling Enterprise Edition Server

### Download {#download-gatling-enterprise}

Gatling Enterprise Edition is packaged as a zip bundle that can be downloaded from our downloads server (only for customers):
...
{{< alert tip >}}
Check the [documentation conventions]({{< ref "../introduction#custom-bundle-download-link" >}}) for ways to find your custom Gatling Enterprise Edition bundle download link.
{{< /alert >}}

On launch, Gatling Enterprise Edition will create or update the Gatling Enterprise Edition schema in the Cassandra database.

### Launch {#launch-gatling-enterprise}

You can launch Gatling Enterprise Edition in the background using the following command:
...
Gatling Enterprise Edition will log its PID and write it to a `pidfile` which names will also be echoed.
...
<1> Provided license key, you should not edit this configuration directly from this file, Gatling Enterprise Edition will ask for it when you launch it or when your current license is expired
...
* <6> For testing purpose, you can make Gatling Enterprise Edition produces a self-signed certificate
...
* <1> Injectors HTTP listening port, so Gatling Enterprise Edition can connect and collect the stats
...
* <1> password for the Gatling Enterprise Edition superAdmin account. Gatling Enterprise Edition will create a new password when you launch it for the first time.
* <2> key for encrypting cookies. Must be 128, 192 or 256 bit (not bytes) long. Gatling Enterprise Edition will create a new secretKey when you launch it for the first time.
...
Fill the following fields if you want to enable OpenID authentication on Gatling Enterprise Edition. All fields need to be uncommented and filled. For more information on our OpenID integration, please check [corresponding section]({{< ref "#openid-authentication" >}}).

* <1> URL for discovery of OpenID endpoints
* <2> Application id received when registering Gatling Enterprise Edition
* <3> Client secret used to authenticate Gatling Enterprise Edition (we don't support secret certificates yet)
...
<1> Url to your Grafana dashboard using the Gatling Enterprise Edition datasource (create a link in the Gatling Enterprise Edition dashboard to the Grafana dashboard)

If you want to modify a value, don't forget to uncomment the line, by deleting the # sign. Any changes to the `frontline.conf` file needs a Gatling Enterprise Edition restart to take effect.
...
- Read permission to unzipped Gatling Enterprise Edition bundle
...
you'll have to configure a different directory without `noexec`. Edit the Gatling Enterprise Edition launch script and pass an additional System properties `-Djna.tmpdir=PATH_TO_DIR_WITHOUT_NOEXEC`.
...
Gatling Enterprise Edition uses the Logback library for logging.
By default, it will log on the filesystem, check logback.xml file.
Feel free to tune the default behavior if needed.

#### LDAP

Gatling Enterprise Edition is able to use LDAP to manage its users. The LDAP mode has been tested with OpenLDAP, and Active Directory servers, but it should work with all regular LDAP implementations.

#### OpenID Authentication

Gatling Enterprise Edition is able to use OpenID connect to manage its users. The OpenID Connect mode has been tested with One Login and Microsoft identity platform.
...
Gatling Enterprise Edition can be configured to automatically delete runs based on max-age and/or max number of runs by simulation.

#### Source Control System Client (typically git)

If you intend to have Gatling Enterprise Edition build tests from sources,
it needs to be able to fetch the test sources from your remote source repository, ie:

- a client for your Source Control System (ex: git, svn, perforce, etc) to be installed on the Gatling Enterprise Edition host
- this client to be in the PATH and executable for the user running the Gatling Enterprise Edition JVM process
...
If you intend to have Gatling Enterprise Edition build tests from sources,
then Gatling Enterprise Edition needs to be able to build the fetched resources, ie:

- a client for your build tool (ex: sbt, maven, gradle, etc) needs to be installed on the Gatling Enterprise Edition host
- this client needs to be in the PATH and executable for the user running the Gatling Enterprise Edition JVM process
